...
{
    QString ret = QString("sk21:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SK21 unit input in lnReqAmount, we generate the URI with SK21 as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(sk21Units::format(sk21Units::SK21, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
